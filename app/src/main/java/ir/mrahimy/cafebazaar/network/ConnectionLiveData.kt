 ...
 *
 * Since this live data is observed in [BaseActivity][app.vijecard.wallet.base.BaseActivity]
 * you can use ConnectionLiveData.value to access the updated ConnectivityModel
 *
 */
object ConnectionLiveData : LiveData<ConnectivityModel>() {
    ...
    private val networkReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                postValue(ConnectivityModel(1, activeNetwork?.isConnected == true))
            } else throw Exception("cannot happen")
        }
    }
}

data class ConnectivityModel(
    val type: Int,
